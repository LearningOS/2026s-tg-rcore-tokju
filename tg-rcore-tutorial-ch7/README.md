...
- [x] 能解释为什么要用统一 `Fd` 枚举承载“文件/管道/标准IO”
- [x] 能从代码说明 `pipe` 建立后父子进程如何通过读写端通信
- [x] 能说明信号的发送、注册、屏蔽、返回四个关键 syscall 语义
- [x] 能解释“syscall 返回前检查信号”对进程退出路径的影响
- [x] 能执行 `./test.sh base` 并通过本章基础测试
...
**源码行号速查（当前 `src/main.rs`）：** `_start` 第 111-127 行（32 页内核栈）；`rust_main` 第 180-278 行；`kernel_space` 第 287 行起；`impls` 第 356-851 行；`stub` 第 855 行起。入口为内联 `_start`，未使用 `boot0!`。
